...
architecture spartan_6 of mii_gmii_io is
    ...
    attribute IODELAY_GROUP of delayctrl_inst: label is "mygroup";
...
begin
    ...
    mii_rx_clk_BUFR_inst : BUFR
       generic map (
      BUFR_DIVIDE => "BYPASS",   -- Values: "BYPASS, 1, 2, 3, 4, 5, 6, 7, 8" 
      SIM_DEVICE => "7SERIES"  -- Must be set to "7SERIES" 
   )
   port map (
      O => clock_mii_rx_div,     -- 1-bit output: Clock output port
      CE => '0',   -- 1-bit input: Active high, clock enable (Divided modes only)
      CLR => '0', -- 1-bit input: Active high, asynchronous clear (Divided modes only)
      I => clock_mii_rx_ibufg      -- 1-bit input: Clock buffer input driven by an IBUF, MMCM or local interconnect
   );
...
--		);
   ...
   delayctrl_inst : IDELAYCTRL
        port map (
           ...
           REFCLK => clock_mii_rx_div, -- 1-bit input: Reference clock input
	...
	mii_rx_dv_buffer_inst : entity work.input_buffer(kintex_7)
		generic map(
			HAS_DELAY    => TRUE,
	...
	mii_rx_er_buffer_inst : entity work.input_buffer(kintex_7)
		generic map(
			HAS_DELAY    => TRUE,
			IDELAY_VALUE => MII_RX_INPUT_DELAY
		)
	...
	mii_rxd_buffer_generate : for i in mii_rxd_i'range generate
		mii_rxd_buffer_inst : entity work.input_buffer(kintex_7)
			generic map(
				HAS_DELAY    => TRUE,
				IDELAY_VALUE => MII_RX_INPUT_DELAY
			)
