...
begin
	...
	delay_gen : if HAS_DELAY = TRUE generate
		fixed_input_delay_inst : entity work.fixed_input_delay(spartan_6)
			generic map(
...
architecture kintex_7 of input_buffer is
	signal delayed : std_ulogic := '0';
...
begin
	-- When delay activated: Instantiate IODELAY2 and then capture its output
	delay_gen : if HAS_DELAY = TRUE generate
		fixed_input_delay_inst : entity work.fixed_input_delay(kintex_7)
			generic map(
				IDELAY_VALUE => IDELAY_VALUE
			)
			port map(
